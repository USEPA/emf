...
public class ExportTaskServiceImpl implements ExportTaskService {
    ...
    private static Log log = LogFactory.getLog(ExportTaskServiceImpl.class);
...
//    public ExportTaskServiceImpl(User user, File file, EmfDataset dataset, Services services, AccessLog accesslog,
//            String rowFilters, String colOrders,
//            DbServerFactory dbFactory, HibernateSessionFactory sessionFactory, Version version, EmfDataset filterDataset, Version filterDatasetVersion, String filterDatasetJoinCondition, boolean download,
//            String taskId, String submitterId) {
//        this.taskId = taskId;
...
//        log.info("this.userDao.all().size() = " + this.userDao.all().size()) ;
        ...
        if (DebugLevels.DEBUG_1())
            System.out.println(">>## ExportTask:run() Task#" + taskId + " for datasetId: " + this.dataset.getId());
        if (DebugLevels.DEBUG_1())
...
//        this.statusServices.add(status);
    ...
    protected void finalize() throws Throwable {
        taskCount--;
        if (DebugLevels.DEBUG_1())
            System.out.println(">>>> Destroying object: Task#" + taskId);
        super.finalize();
    }
