...
public class SQLQACompareVOCSpeciationWithHAPInventoryQueryRunner extends SQLQAProgramRunner {
    
    private String emissioDatasourceName;

    public SQLQACompareVOCSpeciationWithHAPInventoryQueryRunner(DbServer dbServer, EntityManagerFactory entityManagerFactory, QAStep qaStep) {
    ...
    protected String query(DbServer dbServer, QAStep qaStep, String tableName) throws EmfException {
        SQLCompareVOCSpeciationWithHAPInventoryQuery parser = new SQLCompareVOCSpeciationWithHAPInventoryQuery(entityManagerFactory, dbServer, emissioDatasourceName, tableName, qaStep);
    ...
    public void run() throws EmfException{
        ...
        if (programArguments == null || programArguments.trim().length() == 0) {
            throw new EmfException("Please specify the compare CAP/HAP inventories program query");
        }
