...
//    }
    ...
    public void doRemove(ModuleType[] types) throws EmfException {
        ...
        try {
            service().deleteModuleTypes(session.user(), lockedTypes);
    ...
    private ModuleType[] getLockedTypes(ModuleType[] types) throws EmfException{
        ...
        for (int i=0; i < types.length; i++){
            ModuleType locked = service().obtainLockedModuleType(session.user(), types[i]);
            if (locked == null) {
    ...
    private void releaseLocked(ModuleType[] lockedTypes) {
        ...
        for(int i = 0; i < lockedTypes.length; i++) {
            try {
                service().releaseLockedModuleType(session.user(), lockedTypes[i]);
    ...
    public void doRefresh() throws EmfException {
        view.refresh(service().getModuleTypes());
    }
