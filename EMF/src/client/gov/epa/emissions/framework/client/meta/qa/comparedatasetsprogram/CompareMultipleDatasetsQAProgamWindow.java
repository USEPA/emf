...
public class CompareMultipleDatasetsQAProgamWindow extends DisposableInteralFrame implements EditQAEmissionsView {
    
    private AddRemoveDatasetVersionWidget datasetWidgetBase;
    private AddRemoveDatasetVersionWidget[] datasetWidgetCompare = new AddRemoveDatasetVersionWidget[5];
    ...
    private List<DatasetVersion>[] compareDatasetVersions = new ArrayList[5];
    ...
    public CompareMultipleDatasetsQAProgamWindow(DesktopManager desktopManager, String program, 
            EmfSession session, DatasetVersion[] baseDatasetVersions, List<DatasetVersion>[] compareDatasetVersions, 
        ...
        super("Emissions Inventories Editor", new Dimension(1000, 650), desktopManager);
    ...
    private JPanel createLayout(EmfDataset dataset) throws EmfException {
        ...
        JPanel datasetsContent = new JPanel(new SpringLayout());
        SpringLayoutGenerator layoutGenerator = new SpringLayoutGenerator();
       
        layoutGenerator.addLabelWidgetPair("Base datasets:", baseDatasetVersionWidget(), datasetsContent);
        for (int i = 0; i < 5; i++)
            layoutGenerator.addLabelWidgetPair("Compare datasets (" + (i + 1) + "):", compareDatasetVersionWidget(i), datasetsContent);
        
        layoutGenerator.makeCompactGrid(datasetsContent, 6, 2, // rows, cols
                5, 5, // initialX, initialY
                10, 10);// xPad, yPad*/
        content.add(datasetsContent);
        
        JPanel optionsContent = new JPanel(new SpringLayout());
        ...
        scrollableComment.setPreferredSize(new Dimension(300, 75));
        layoutGenerator.addLabelWidgetPair("Group By Expressions:", scrollableComment, optionsContent);
        ...
        scrollableComment2.setPreferredSize(new Dimension(300, 75));
        layoutGenerator.addLabelWidgetPair("Aggregate Expressions:", scrollableComment2, optionsContent);
        ...
        scrollableComment3.setPreferredSize(new Dimension(300, 75));
        layoutGenerator.addLabelWidgetPair("Matching Expressions:", scrollableComment3, optionsContent);

        joinTypeCombo();
        layoutGenerator.addLabelWidgetPair("Join Type:", joinTypes, optionsContent);
        ...
        scrollableComment4.setPreferredSize(new Dimension(300, 75));
        layoutGenerator.addLabelWidgetPair("Where Filter:", scrollableComment4, optionsContent);
        ...
        scrollableComment5.setPreferredSize(new Dimension(300, 75));
        layoutGenerator.addLabelWidgetPair("Base Field Suffix :", scrollableComment5, optionsContent);
        ...
        scrollableComment6.setPreferredSize(new Dimension(300, 75));
        layoutGenerator.addLabelWidgetPair("Compare Field Suffix :", scrollableComment6, optionsContent);
        
        layoutGenerator.makeCompactGrid(optionsContent, 7, 2, // rows, cols
                5, 5, // initialX, initialY
                10, 10);// xPad, yPad*/
    ...
    private void joinTypeCombo() {
        ...
        joinTypes.setPreferredSize(new Dimension(300, 25));
    ...
    private JPanel compareDatasetVersionWidget(int i) throws EmfException {
        datasetWidgetCompare[i] = new AddRemoveDatasetVersionWidget(false, 0, this, parentConsole, session);
        datasetWidgetCompare[i].setPreferredSize(new Dimension(350,220));
        datasetWidgetCompare[i].setDefaultDatasetType(this.defaultDatasetType);
        List<DatasetVersion> datasetVersions = new ArrayList<DatasetVersion>();
        if(compareDatasetVersions[i].size() > 0) {
            
            for (DatasetVersion datasetVersion : compareDatasetVersions[i]) {
                datasetVersions.add(datasetVersion);
            }
        }
        datasetWidgetCompare[i].setDatasetVersions(datasetVersions.toArray(new DatasetVersion[0]));
        return datasetWidgetCompare[i];
    }
    ...
    private Action okAction() {
        return new AbstractAction() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (datasetWidgetCompare[0].getDatasetVersions().length == 0) {
                    messagePanel.setError("Please choose at least one dataset to compare with.");
                    return;
                }
...
*/
                ...
                //compare tag
                for (int i = 0; i < 5; i++) {
                    programArguments.append(EditQAStepWindow.COMPARE_TAG + (i + 1) + "\n");
                    for (Object datasetVersion : datasetWidgetCompare[i].getDatasetVersions()) {
                        programArguments.append(((DatasetVersion)datasetVersion).getDataset().getName() + "|" + ((DatasetVersion)datasetVersion).getVersion().getVersion() + "\n");
                    }
                }
                //group by tag
