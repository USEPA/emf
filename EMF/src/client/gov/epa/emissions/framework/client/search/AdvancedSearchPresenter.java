...
import gov.epa.emissions.framework.client.meta.*;
...
public class AdvancedSearchPresenter<F, M> implements RefreshObserver, SearchObserver<F, M> {

    private AdvancedSearchView<F,M> view;

    private EmfSession session;

    private final Class<M> persistentClass;

    public AdvancedSearchPresenter(EmfSession session, final Class<M> persistentClass) {
    ...
    public void doDisplay(AdvancedSearchView<F,M> view) {
        this.view = view;
        view.observe(this);
        view.display();
    }

    public AdvancedSearchPresenter(final Class<M> persistentClass) {
    ...
    public void doRefresh() throws EmfException {
        view.refresh((M[]) Array.newInstance(persistentClass, 0));
        view.clearMessage();
    }
...
//    }
    ...
    @Override
    public M[] doSearch(F filter) throws EmfException {
        return null;
    }
}
