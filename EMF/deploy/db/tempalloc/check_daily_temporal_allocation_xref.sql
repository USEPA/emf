CREATE OR REPLACE FUNCTION public.check_daily_temporal_allocation_xref(
...
BEGIN
  ...
  INSERT INTO emissions.' || messages_table_name || ' (
   ...
   SELECT ' || messages_dataset_id || ',
          ...
          ' || inv_stackid || ',
          ' || inv_processid || ',
          ''No weekly/daily profile assigned to source''
...
LEFT JOIN temp_alloc_xref xref
  ...
  -- get the weekly or daily profile dataset id and version
  SELECT ta.daily_profile_dataset_id IS NOT NULL
    INTO use_daily_profile
    FROM emf.temporal_allocation ta
   WHERE ta.id = temporal_allocation_id;

  IF use_daily_profile THEN
    SELECT ta.daily_profile_dataset_id,
