...
CREATE OR REPLACE FUNCTION public.populate_table_consolidation_tracker() RETURNS void AS $$
DECLARE
...
BEGIN
	...
	IF has_table_consolidation_tracker_table THEN
		...
		truncate emf.table_consolidation_tracker;
		raise notice '%', 'Drop emf.table_consolidation_tracker table';
		drop table emf.table_consolidation_tracker;
	END IF;
	raise notice '%', 'Create emf.table_consolidation_tracker table';
	-- create table
	create table emf.table_consolidation_tracker (
		dataset_id int4, 
		...
		column_name_list text NOT NULL DEFAULT '', 
		column_data_type_list text NOT NULL DEFAULT '',
	...
	-- populate table
	insert into emf.table_consolidation_tracker (
			dataset_id, 
			dataset_name,
			dataset_type_id, 
			dataset_type,
	...
	where d.status <> 'Deleted'
		...
		and lower(i.table_name) not in (
			select lower(output_table)
			from emf.table_consolidations
		)
		-- don't include tables already consolidated 2nd approach, look in internal sources...
		and lower(i.table_name) not in (
			select lower(i.table_name) 
			from emf.internal_sources i
			group by lower(i.table_name)
			having count(1) > 1
		);
	...
	delete from emf.table_consolidation_tracker 
	where dataset_id in (
		select dataset_id
		from emf.table_consolidation_tracker tct
			...
			inner join (
				...
				from emf.table_consolidation_tracker
					inner join emf.dataset_types dt
					on dt.id = emf.table_consolidation_tracker.dataset_type_id
	...
	update emf.table_consolidation_tracker tct
		...
		from emf.table_consolidation_tracker tct
			...
			inner join (
				select distinct on (case when dt.importer_classname = 'gov.epa.emissions.commons.io.generic.LineImporter' then dt.name else '' end, column_name_list, column_data_type_list)
					case 
						when length('ds_consol_' || dataset_old_table || round(EXTRACT(EPOCH FROM now()))) <= 63 then 
							'ds_consol_' || dataset_old_table || round(EXTRACT(EPOCH FROM now()))
						else 
							substring('ds_consol_' || dataset_old_table, 1, 63 - length(round(EXTRACT(EPOCH FROM now())) || '')) || round(EXTRACT(EPOCH FROM now()))
...
--							substring('ds_consol_' || dataset_name, 1, 63 - length(round(EXTRACT(EPOCH FROM now())) || '')) || round(EXTRACT(EPOCH FROM now()))
	...
	raise notice '%', 'Total records in emf.table_consolidation_tracker = ' || cnt;
	...
	LOOP
		cnter := cnter + 1;
		raise notice '%', 'Update emf.table_consolidation_tracker ' || table_consolidation_tracker_record.dataset_old_table || ' table record count, ' || cnter;
		...
		--set record_count = coalesce((SELECT reltuples FROM pg_class r WHERE relkind = ''r'' AND relname = ''' || lower(table_consolidation_tracker_record.dataset_old_table) || '''), 0)
...
select public.populate_table_consolidation_tracker();
