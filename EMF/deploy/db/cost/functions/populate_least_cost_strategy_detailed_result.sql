...
--			limit ' || (apply_order) || '
	...
	execute  '/*truncate emissions.'|| detailed_result_table_name || ';*/
		...
		scc,
		region_cd,
		facility_id, unit_id, rel_point_id, process_id, 
	...
	select 	
		...
		ctl_ann_cost_per_ton as computed_ctl_cost_per_ton,
		...
		input_emis,
		output_emis,
		substr(region_cd, 1, 2),
		substr(region_cd, 3, 3),
		sic,
		naics,
		source_id,
		' || input_dataset_id || '::integer,
	...
	IF coalesce(costcurve_table_name, '') <> '' THEN
		execute 'insert into emissions.'|| costcurve_table_name || ' 
			...
			select ' || costcurve_dataset_id || ',
				dr.poll, 
				sum(dr.annual_cost), 
				sum(dr.annual_cost) / sum(dr.eff_emis_reduction), 
				...
				case when dr.poll = ANY (''' || target_pollutant_names::varchar || ''') then sum(dr.eff_emis_reduction) / ' || uncontrolled_emis || ' * 100 else null::double precision end, 
				sum(dr.eff_emis_reduction), 
