				...
				when cr.source_id is null and cont.source_id is not null and cont.percent_reduction <> 0.0 then 
					case 
						...
						when cont.replacement_addon = ''A'' then 
							least(cont.percent_reduction + coalesce(inv.ann_pct_red,0.0), 100.0)
				...
				when cr.source_id is null and cont.source_id is not null and cont.jan_pct_red <> 0.0 then 
					case 
						when cont.replacement_addon = ''R'' then 
							cont.percent_reduction
						when cont.replacement_addon = ''A'' then 
							least(cont.jan_pct_red + coalesce(inv.jan_pctred,0.0), 100.0)
						else inv.jan_pctred 
					end 
				when cr.source_id is not null then null::double precision 
				else inv.jan_pctred 
				...
				when cr.source_id is null and cont.source_id is not null and cont.feb_pct_red <> 0.0 then 
					case 
						when cont.replacement_addon = ''R'' then 
							cont.feb_pct_red
						when cont.replacement_addon = ''A'' then 
							least(cont.feb_pct_red + coalesce(inv.feb_pctred,0.0), 100.0)
						else inv.feb_pctred 
					end 
				when cr.source_id is not null then null::double precision 
				else inv.feb_pctred 
				...
				when cr.source_id is null and cont.source_id is not null and cont.mar_pct_red <> 0.0 then 
					case 
						when cont.replacement_addon = ''R'' then 
							cont.mar_pct_red
						when cont.replacement_addon = ''A'' then 
							least(cont.mar_pct_red + coalesce(inv.mar_pctred,0.0), 100.0)
						else inv.mar_pctred 
					end 
				when cr.source_id is not null then null::double precision 
				else inv.mar_pctred 
				...
				when cr.source_id is null and cont.source_id is not null and cont.apr_pct_red <> 0.0 then 
					case 
						when cont.replacement_addon = ''R'' then 
							cont.apr_pct_red
						when cont.replacement_addon = ''A'' then 
							least(cont.apr_pct_red + coalesce(inv.apr_pctred,0.0), 100.0)
						else inv.apr_pctred 
					end 
				when cr.source_id is not null then null::double precision 
				else inv.apr_pctred 
				...
				when cr.source_id is null and cont.source_id is not null and cont.may_pct_red <> 0.0 then 
					case 
						when cont.replacement_addon = ''R'' then 
							cont.may_pct_red
						when cont.replacement_addon = ''A'' then 
							least(cont.may_pct_red + coalesce(inv.may_pctred,0.0), 100.0)
						else inv.may_pctred 
					end 
				when cr.source_id is not null then null::double precision 
				else inv.may_pctred 
				...
				when cr.source_id is null and cont.source_id is not null and cont.jun_pct_red <> 0.0 then 
					case 
						when cont.replacement_addon = ''R'' then 
							cont.jun_pct_red
						when cont.replacement_addon = ''A'' then 
							least(cont.jun_pct_red + coalesce(inv.jun_pctred,0.0), 100.0)
						else inv.jun_pctred 
					end 
				when cr.source_id is not null then null::double precision 
				else inv.jun_pctred 
				...
				when cr.source_id is null and cont.source_id is not null and cont.jul_pct_red <> 0.0 then 
					case 
						when cont.replacement_addon = ''R'' then 
							cont.jul_pct_red
						when cont.replacement_addon = ''A'' then 
							least(cont.jul_pct_red + coalesce(inv.jul_pctred,0.0), 100.0)
						else inv.jul_pctred 
					end 
				when cr.source_id is not null then null::double precision 
				else inv.jul_pctred 
				...
				when cr.source_id is null and cont.source_id is not null and cont.aug_pct_red <> 0.0 then 
					case 
						when cont.replacement_addon = ''R'' then 
							cont.aug_pct_red
						when cont.replacement_addon = ''A'' then 
							least(cont.aug_pct_red + coalesce(inv.aug_pctred,0.0), 100.0)
						else inv.aug_pctred 
					end 
				when cr.source_id is not null then null::double precision 
				else inv.aug_pctred 
				...
				when cr.source_id is null and cont.source_id is not null and cont.sep_pct_red <> 0.0 then 
					case 
						when cont.replacement_addon = ''R'' then 
							cont.sep_pct_red
						when cont.replacement_addon = ''A'' then 
							least(cont.sep_pct_red + coalesce(inv.sep_pctred,0.0), 100.0)
						else inv.sep_pctred 
					end 
				when cr.source_id is not null then null::double precision 
				else inv.sep_pctred 
				...
				when cr.source_id is null and cont.source_id is not null and cont.oct_pct_red <> 0.0 then 
					case 
						when cont.replacement_addon = ''R'' then 
							cont.oct_pct_red
						when cont.replacement_addon = ''A'' then 
							least(cont.oct_pct_red + coalesce(inv.oct_pctred,0.0), 100.0)
						else inv.oct_pctred 
					end 
				when cr.source_id is not null then null::double precision 
				else inv.oct_pctred 
				...
				when cr.source_id is null and cont.source_id is not null and cont.nov_pct_red <> 0.0 then 
					case 
						when cont.replacement_addon = ''R'' then 
							cont.nov_pct_red
						when cont.replacement_addon = ''A'' then 
							least(cont.nov_pct_red + coalesce(inv.nov_pctred,0.0), 100.0)
						else inv.nov_pctred 
					end 
				when cr.source_id is not null then null::double precision 
				else inv.nov_pctred 
				...
				when cr.source_id is null and cont.source_id is not null and cont.dec_pct_red <> 0.0 then 
					case 
						when cont.replacement_addon = ''R'' then 
							cont.dec_pct_red
						when cont.replacement_addon = ''A'' then 
							least(cont.dec_pct_red + coalesce(inv.dec_pctred,0.0), 100.0)
						else inv.dec_pctred 
					end 
				when cr.source_id is not null then null::double precision 
				else inv.dec_pctred 
