# Dataset Quality Assurance # {#sec:qa_chapter}
...
A dataset can have many QA steps associated with it. To view a dataset's QA steps, first select the dataset in the Dataset Manager and click the Edit Properties button. Switch to the QA tab to see the list of QA steps as in [@Fig:dataset_qa_steps].

![QA Steps for a Dataset](images/dataset_properties_qa_nonroad.png){#fig:dataset_qa_steps}
...
Each dataset type can have predefined QA steps called QA Step Templates. QA step templates can be added to a dataset type and configured by EMF Administrators using the Dataset Type Manager (see [@Sec:dataset_types_section]). QA step templates are easy to run for a dataset because they've already been configured.

To see a list of available QA step templates for your dataset, open your dataset's QA tab in the Dataset Properties Editor ([@Fig:dataset_qa_steps]). Click the Add from Template button to open the Add QA Steps dialog. [@Fig:add_qa_step_template_nonroad] shows the available QA step templates for an ORL Nonroad Inventory.

![Add QA Steps From Template](images/add_qa_step_template_nonroad.png){#fig:add_qa_step_template_nonroad}
...
The newly added QA step is now shown in the list of QA steps for the dataset ([@Fig:qa_steps_with_new_step]).

![QA Steps with New Step Added](images/qa_steps_with_new_step.png){#fig:qa_steps_with_new_step}

To see the details of the QA step, select the step and click the Edit button. This brings up the Edit QA Step window like [@Fig:edit_new_qa_step_from_template].

![Edit New QA Step from Template](images/edit_new_qa_step_from_template.png){#fig:edit_new_qa_step_from_template}
...
The Arguments textbox shows the arguments used by the QA step program. In this case, the QA step is a SQL query and the Arguments field shows the query that will be run. The special SQL syntax used for QA steps is discussed in [@Sec:qa_step_sql_syntax_section].
...
The Edit QA Step window also includes options for exporting the results of a QA step to a file. This is described in [@Sec:export_qa_results_section].

At this point, the next step is to actually run the QA step as described in [@Sec:running_steps_section].

## Adding Custom QA Steps ## {#sec:add_custom_qa_section}

In addition to using QA steps from templates, you can define your own custom QA steps. From the QA tab of the Dataset Properties Editor ([@Fig:dataset_qa_steps]), click the Add Custom button to bring up the Add Custom QA Step dialog as shown in [@Fig:add_custom_qa_step].

![Add Custom QA Step Dialog](images/add_custom_qa_step.png){#fig:add_custom_qa_step}
...
[@Sec:avg_day_to_ann_section] shows how to create a custom QA step that uses the built-in QA program "Average day to Annual Inventory" to calculate annual emissions from average-day emissions. [@Sec:compare_datasets_section] demonstrates using the Compare Datasets QA program to compare two inventories. [@Sec:create_custom_sql_qa_section] gives an example of creating a custom QA step based on a SQL query from an existing QA step.

## Running QA Steps ## {#sec:running_steps_section}

To run a QA step, open the QA tab of the Dataset Properties Editor and select the QA step you want to run as shown in [@Fig:select_qa_step_to_run].

![Select a QA Step to Run](images/select_qa_step_to_run.png){#fig:select_qa_step_to_run}
...
In the QA tab, click the Refresh button to update the table of QA steps as shown in [@Fig:refreshed_qa_steps].

![Refreshed QA Steps](images/refreshed_qa_steps.png){#fig:refreshed_qa_steps}
...
To view the results of the QA step, select the step in the QA tab and click the View Results button. A dialog like [@Fig:view_qa_results_records] will pop-up asking how many records of the results you would like to preview.

![View QA Results: Select Number of Records](images/view_qa_results_records.png){#fig:view_qa_results_records}

Enter the number of records to view or click the View All button to see all records. The View QA Step Results window will display the results of the QA step as shown in [@Fig:view_qa_results].

![View QA Results](images/view_qa_results.png){#fig:view_qa_results}

## Exporting QA Step Results ## {#sec:export_qa_results_section}
...
To export the results of a QA step, select the QA step of interest in the QA tab of the Dataset Properties Editor. Then click the Edit button to bring up the Edit QA Step window as shown in [@Fig:export_qa_step_results].

![Export QA Step Results](images/export_qa_step_results.png){#fig:export_qa_step_results}

Typically, you will want to check the **Download result file to local machine?** checkbox so the exported file will automatically be downloaded to your local machine. You can type in a name for the exported file in the Export Name field. Then click the Export button. If you did not enter an Export Name, the application will confirm that you want to use an auto-generated name with the dialog shown in [@Fig:no_export_name].

![Export Name Not Specified](images/no_export_name.png){#fig:no_export_name}

Next, you'll see the Export QA Step Results customization window ([@Fig:export_qa_step_results_2]).

![Export QA Step Results Customization Window](images/export_qa_step_results_2.png){#fig:export_qa_step_results_2}

The Row Filter textbox allows you to limit which rows of the QA step results to include in the exported file. [@Tbl:row_filter_syntax_table] provides some examples of the syntax used by the row filter. Available Columns lists the column names from the results that could be used in a row filter. In [@Fig:export_qa_step_results_2], the columns `fips`, `poll`, and `ann_emis` are available. To export only the results for counties in North Carolina (state FIPS code = 37), the row filter would be `fips like '37%'`.

Click the Finish button to start the export. At the top of the Edit QA Step window, you'll see the message "Started Export. Please monitor the Status window to track your export request." like [@Fig:export_qa_step_results_started]

![Export QA Step Results Started](images/export_qa_step_results_started.png){#fig:export_qa_step_results_started}
...
You can bring up the Downloads window as shown in [@Fig:downloads_qa_step_results] by opening the **Window** menu at the top of the EMF main window and selecting **Downloads**.

![Downloads Window: QA Step Results](images/downloads_qa_step_results.png){#fig:downloads_qa_step_results}
...
To export a KMZ file that can be loaded into Google Earth, you will first need to view the results of the QA step. You can view a QA step's results by either selecting the QA step in the QA tab of the Dataset Properties Editor (see [@Fig:dataset_qa_steps]) and then clicking the View Results button, or you can click View Results from the Edit QA Step window. [@Fig:view_qa_step_results_latlon] shows the View QA Step Results window for a summary by county and pollutant with descriptions. The summary includes latitude and longitude values for each county.

![View QA Step Results with Latitude and Longitude Values](images/view_qa_step_results_latlon.png){#fig:view_qa_step_results_latlon}

From the **File** menu in the top left corner of the View QA Step Results window, select **Google Earth**. Make sure to look at the File menu for the View QA Step Results window, not the main EMF application. The Create Google Earth file window will be displayed as shown in [@Fig:create_google_earth_file].

![Create Google Earth File](images/create_google_earth_file.png){#fig:create_google_earth_file}
...
## Average Day to Annual Inventory QA Program ## {#sec:avg_day_to_ann_section}
...
After determining which dataset to add the QA step to, create a new custom QA step as described in [@Sec:add_custom_qa_section]. [@Fig:custom_qa_step_avg_day] shows the Add Custom QA Step dialog. We've entered a name for the step and used the **Program** pull-down menu to select "Average day to Annual Inventory".

![Add Custom QA Step Using Average Day to Annual Inventory QA Program](images/custom_qa_step_avg_day.png){#fig:custom_qa_step_avg_day}

"Average day to Annual Inventory" is a QA program built into the EMF that takes a set of average-day emissions inventories as input and outputs an annual inventory by calculating monthly total emissions and summing all months. Click the OK button in the Add Custom QA Step dialog to save the new QA step. We'll enter the QA program arguments in a minute. Back in the QA tab of the Dataset Properties Editor, select the newly created QA step and click Edit to open the Edit QA Step window shown in [@Fig:edit_custom_qa_step].

![Edit Custom QA Step](images/edit_custom_qa_step.png){#fig:edit_custom_qa_step}

We need to define the arguments that will be sent to the QA program that this QA step will run. The QA program is "Average day to Annual Inventory" so the arguments will be a list of month-specific inventories. Click the **Set** button to the right of the Arguments box to open the Set Inventories dialog as shown in [@Fig:custom_qa_step_set_inventories].

![Set Inventories for Average Day to Annual Inventory QA Program](images/custom_qa_step_set_inventories.png){#fig:custom_qa_step_set_inventories}

The Set Inventories dialog is specific to the "Average day to Annual Inventory" QA program. Other QA programs have different dialogs for setting up their arguments. The January inventory that we added the QA step to is already listed. We need to add the other 11 month-specific inventory files. Click the Add button to open the Select Datasets dialog shown in [@Fig:custom_qa_step_choose_invs].

![Select Datasets for QA Program](images/custom_qa_step_choose_invs.png){#fig:custom_qa_step_choose_invs}

In the Select Datasets dialog, the dataset type is automatically set to ORL Nonroad Inventory (ARINV) matching our January inventory. The other ORL nonroad inventory datasets are shown in a list. We can use the **Dataset name contains:** field to enter a search term to narrow the list. We're using 2005 inventories so we'll enter `2005` as our search term to match only those datasets whose name contains "2005". Then we'll select all the inventories in the list as shown in [@Fig:custom_qa_step_choose_invs_2]. 
...
![Select Filtered Datasets for QA Program](images/custom_qa_step_choose_invs_2.png){#fig:custom_qa_step_choose_invs_2}

Click the OK button in the Select Datasets dialog to save the selected inventories and return to the Set Inventories dialog. As shown in [@Fig:custom_qa_step_invs_set], the list of emission inventories now contains all 12 month-specific datasets.

![Inventories for Average Day to Annual Inventory QA Program](images/custom_qa_step_invs_set.png){#fig:custom_qa_step_invs_set}

Click the OK button in the Set Inventories dialog to return to the Edit QA Step window shown in [@Fig:custom_qa_step_arguments]. The Arguments textbox now lists the 12 month-specific inventories and the flag (-inventories) needed for the "Average day to Annual Inventory" QA program.

![Custom QA Step with Arguments Set](images/custom_qa_step_arguments.png){#fig:custom_qa_step_arguments}

Click the Save button at the bottom of the Edit QA Step window to save the QA step. This QA step can now be run as described in [@Sec:running_steps_section].

## Compare Datasets QA Program ## {#sec:compare_datasets_section}
...
First, we'll select a dataset to associate the QA step with. In this example, we'll be comparing January and February emissions using the January dataset as the base inventory. The EMF doesn't dictate which dataset should have the QA step associated with it so we'll choose the base dataset as a convention. From the Dataset Manager, select the January inventory (shown in [@Fig:qa_compare_dataset_manager]) and click the **Edit Properties** button.

![Select Dataset to Add QA Step](images/qa_compare_dataset_manager.png){#fig:qa_compare_dataset_manager}

Open the QA tab (shown in [@Fig:qa_compare_qa_tab]) and click **Add Custom** to add a new QA step.

![Dataset Editor QA Tab for Selected Dataset](images/qa_compare_qa_tab.png){#fig:qa_compare_qa_tab}

In the Add Custom QA Step dialog shown in [@Fig:qa_compare_add_custom_step], enter a name for the new QA step like "Compare to February". Use the **Program** pull-down menu to select the QA program "Compare Datasets".

![Select QA Program for New QA Step](images/qa_compare_add_custom_step.png){#fig:qa_compare_add_custom_step}

You can enter a description of the QA step as shown in [@Fig:qa_compare_add_custom_step_description]. Then click **OK** to save the QA step. We'll be setting up the arguments to the Compare Datasets QA program in just a minute.

![Add Description to New QA Step](images/qa_compare_add_custom_step_description.png){#fig:qa_compare_add_custom_step_description}

Back in the QA tab of the Dataset Properties Editor, select the newly created QA step and click the **Edit** button (see [@Fig:qa_compare_select_new_step]).

![Select New QA Step from QA Tab](images/qa_compare_select_new_step.png){#fig:qa_compare_select_new_step}

In the Edit QA Step window (shown in [@Fig:qa_compare_edit_step]), click the **Set** button to the right of the **Arguments** textbox.

![Edit New QA Step](images/qa_compare_edit_step.png){#fig:qa_compare_edit_step}

A custom dialog is displayed ([@Fig:qa_compare_set_arguments]) to help you set up the arguments needed by the Compare Datasets QA program.

![Set Up Compare Datasets QA Step](images/qa_compare_set_arguments.png){#fig:qa_compare_set_arguments}

To get started, we'll set the base datasets. Click the **Add** button underneath the **Base Datasets** area to bring up the Select Datasets dialog shown in [@Fig:qa_compare_select_base_dataset].

![Select Base Datasets](images/qa_compare_select_base_dataset.png){#fig:qa_compare_select_base_dataset}

Select one or more datasets to use as the base datasets in the comparison. For this example, we'll select the January inventory by clicking on the dataset name. Then click **OK** to close the dialog and return to the setup dialog. The setup dialog now shows the selected base dataset as in [@Fig:qa_compare_base_dataset_set].

![Base Dataset Set for Compare Datasets](images/qa_compare_base_dataset_set.png){#fig:qa_compare_base_dataset_set}

Next, we'll add the dataset we want to compare against by clicking the **Add** button underneath the **Compare Datasets** area. The Select Datasets dialog is displayed like in [@Fig:qa_compare_select_compare_dataset]. We'll select the February inventory and click the **OK** button.

![Select Compare Datasets](images/qa_compare_select_compare_dataset.png){#fig:qa_compare_select_compare_dataset}

Returning to the setup dialog, the comparison dataset is now set as shown in [@Fig:qa_compare_compare_dataset_set].

![Compare Dataset Set for Compare Datasets](images/qa_compare_compare_dataset_set.png){#fig:qa_compare_compare_dataset_set}

The list of base and comparison datasets includes which version of the data will be used in the QA step. For example, the base dataset **2007JanORLTotMARAMAv3.txt [0 (Initial Version)]** indicates that version 0 (named "Initial Version") will be used. When you select the base and comparison datasets, the EMF automatically uses each dataset's Default Version. If any of the datasets have a different version that you would like to use for the QA step, select the dataset name and then click the **Set Version** button underneath the selected dataset. The Set Version dialog shown in [@Fig:qa_compare_set_version] lets you pick which version of the dataset you would like to use.

![Set Dataset Version for Compare Datasets QA Program](images/qa_compare_set_version.png){#fig:qa_compare_set_version}

Next, we need to tell the Compare Datasets QA program how to compare the two datasets. We're going to sum the average-day emissions in each dataset by SCC and pollutant and then compare the results from January to February. In the ORL Nonroad Inventory dataset type, the SCCs are stored in a field called `scc`, the pollutant codes are stored in a column named `poll`, and the average-day emissions are stored in a field called `avd_emis`. In the **Group By Expressions** textbox, type `scc`, press **Enter**, and then type `poll`. In the **Aggregate Expressions** textbox, type `avd_emis`. [@Fig:qa_compare_arguments_set] shows the setup dialog with the arguments entered.

![Arguments Set for Compare Datasets](images/qa_compare_arguments_set.png){#fig:qa_compare_arguments_set}

In this example, we're comparing two datasets of the same type (ORL Nonroad Inventory). This means that the data field names will be consistent between the base and comparison datasets. When you compare datasets with different types, the field names might not match. The **Matching Expressions** textbox allows you to define how the fields from the base dataset should be matched to the comparison dataset. For this case, we don't need to enter anything in the **Matching Expressions** textbox or any of the remaining fields in the setup dialog. The Compare Datasets arguments are described in more detail in [@Sec:compare_datasets_details_section].

In the setup dialog, click **OK** to save the arguments and return to the Edit QA Step window. The **Arguments** textbox now lists the arguments that we set up in the previous step (see [@Fig:qa_compare_step_ready]).

![QA Step with Arguments Set](images/qa_compare_step_ready.png){#fig:qa_compare_step_ready}

The QA step is now ready to run. Click the **Run** button to start running the QA step. A message is displayed at the top of the window as shown in [@Fig:qa_compare_step_running].

![Started Running QA Step](images/qa_compare_step_running.png){#fig:qa_compare_step_running}

In the Status window, you'll see a message about starting to run the QA step followed by a completion message once the QA step has finished running. [@Fig:qa_compare_status_window] shows the two status messages.

![QA Step Running in Status Window](images/qa_compare_status_window.png){#fig:qa_compare_status_window}
...
is displayed, the QA step has finished running. In the Edit QA Step window, click the **Refresh** button to display the latest information about the QA step. The fields **Run Status** and **Run Date** will be populated with the latest run information as shown in [@Fig:qa_compare_step_run_complete].

![QA Step with Run Status](images/qa_compare_step_run_complete.png){#fig:qa_compare_step_run_complete}

Now, we can view the QA step results or export the results. First, we'll view the results inside the EMF client. Click the **View Results** button to open the View QA Step Results window as shown in [@Fig:qa_compare_view_results].

![View Compare Datasets QA Step Results](images/qa_compare_view_results.png){#fig:qa_compare_view_results}

[@Tbl:qa_compare_columns_table] describes each column in the QA step results.
...
: QA Step Results Columns {#tbl:qa_compare_columns_table}

To export the QA step results, return to the Edit QA Step window as shown in [@Fig:qa_compare_ready_export]. Select the checkbox labeled **Download result file to local machine?**. In this example, we have entered an optional **Export Name** for the output file. If you don't enter an **Export Name**, the output file will use an auto-generated name. Click the **Export** button.

![Ready to Export QA Step Results](images/qa_compare_ready_export.png){#fig:qa_compare_ready_export}

The Export QA Step Results dialog will be displayed as shown in [@Fig:qa_compare_export_options]. For more information about the **Row Filter** option, see [@Sec:export_qa_results_section]. To export all the result records, click the **Finish** button.

![Export QA Step Results Options](images/qa_compare_export_options.png){#fig:qa_compare_export_options}

Back in the Edit QA Step window, a message is displayed at the top of the window indicating that the export has started. See [@Fig:qa_compare_export_started].

![Export Started for QA Step Results](images/qa_compare_export_started.png){#fig:qa_compare_export_started}

Check the Status window to see the status of the export as shown in [@Fig:qa_compare_export_status_window].

![Export Messages in Status Window](images/qa_compare_export_status_window.png){#fig:qa_compare_export_status_window}

Once the export is complete, the file will start downloading to your computer. Open the Downloads window to check the download status. Once the progress bar reaches 100%, the download is complete. Right click on the results file and select **Open Containing Folder** as shown in [@Fig:qa_compare_open_download].

![QA Step Results in Downloads Window](images/qa_compare_open_download.png){#fig:qa_compare_open_download}

[@Fig:qa_compare_containing_folder] shows the downloaded file in Windows Explorer. By default, files are downloaded to a temporary directory on your computer. Some disk cleanup programs can automatically delete files in temporary directories; you should move any downloads you want to keep to a more permanent location on your computer.

![Downloaded QA Step Results in Windows Explorer](images/qa_compare_containing_folder.png){#fig:qa_compare_containing_folder}

The downloaded file is a CSV (comma-separated values) file which can be opened in Microsoft Excel or other spreadsheet programs. Double-click the filename to open the file. [@Fig:qa_compare_excel] shows the QA step results in Microsoft Excel.

![Downloaded QA Step Results in Microsoft Excel](images/qa_compare_excel.png){#fig:qa_compare_excel}

### Details of Compare Datasets Arguments ### {#sec:compare_datasets_details_section}
...
![Compare Datasets Example 1](images/qa_compare_example_1.png){#fig:qa_compare_example_1}

[@Fig:qa_compare_example_2] shows the setup dialog for a second example of the Compare Datasets QA program. This example takes a set of ORL nonpoint datasets and compares it to a single FF10 nonpoint inventory. We are grouping by state (first two digits of the FIPS code) and pollutant. A mapping expression is needed between the ORL column `fips` and the FF10 column `region_cd`:
...
![Compare Datasets Example 2](images/qa_compare_example_2.png){#fig:qa_compare_example_2}

## Creating a Custom SQL QA Step ## {#sec:create_custom_sql_qa_section}
...
First, let's look at the structure of a SQL QA step created from a QA step template. [@Fig:custom_qa_step_intro] shows a QA step that generates a summary of the annual emissions by county and pollutant.

![QA Step Reference](images/edit_new_qa_step_from_template.png){#fig:custom_qa_step_intro}
...
For the ORL nonroad inventory dataset type, the annual emission values are stored in a database column named `ann_emis` while the average-day emissions are in a column named `avd_emis`. For any dataset you can see the names of the underlying data columns by viewing the raw data as described in [@Sec:viewing_data_section].
...
Once we know what SQL query to run, we'll create a custom QA step. [@Sec:add_custom_qa_section] describes how to add a custom QA step to a dataset. [@Fig:custom_qa_step_sql] shows the new custom QA step with a name assigned and the Program pull-down menu set to SQL so that the custom QA step will run a SQL query. Our custom SQL query is pasted into the Arguments textbox.

![Custom SQL QA Step Setup](images/custom_qa_step_sql.png){#fig:custom_qa_step_sql}

Click the **OK** button to save the QA step. The newly added QA step is now shown in the list of QA steps for the dataset ([@Fig:custom_qa_step_sql_done]).

![Custom SQL QA Step Ready](images/custom_qa_step_sql_done.png){#fig:custom_qa_step_sql_done}

At this point, you can run the QA step as described in [@Sec:running_steps_section] and view and export the QA step results ([@Sec:export_qa_results_section]) just like any other QA step.
...
## Special SQL Syntax for QA Steps ## {#sec:qa_step_sql_syntax_section}
