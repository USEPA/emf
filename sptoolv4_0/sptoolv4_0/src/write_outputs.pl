...
my $autoImportOutputs = $ENV{"AUTOMATICALLY_IMPORT_OUTPUT"};;
...
else 
{
        ...
        $conn = DBI->connect("DBI:$dbiPG:dbname=$dbname;host=127.0.0.1","$userName","$pwd") 
                or die "Database connection not made: $DBI::errstr\n";
}
...
if ($autoImportOutputs eq "Y")
{
	output_emf($outGSpro,"Chemical Speciation Profiles (GSPRO)");
}
...
if ($autoImportOutputs eq "Y")
{
	output_emf($outGScnv,"Pollutant to Pollutant Conversion (GSCNV)");
}
