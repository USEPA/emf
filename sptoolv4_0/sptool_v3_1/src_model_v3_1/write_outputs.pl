...
if ( $^O eq 'MSWin32' || $^O eq 'dos' )
{
        ...
        $conn = DBI->connect("dbi:PgPP:dbname=$dbname;host=localhost","ozl","") 
...
while (@data = $sth->fetchrow_array())
{
	...
	$dataval    = $data[1];
        ...
        if ( $keyword eq "VOC_TYPE" ) { $runtype = $dataval; }
        if ( $keyword eq "OC_TYPE" )  { $runpmtype = $dataval; }
...
if ( $runout eq "PM" )
   { $outExt = "PM_${runmech}_${runaqm}_$runDate"; }
