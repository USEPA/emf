...
if ( $^O eq 'MSWin32' || $^O eq 'dos' )
{
        ...
        $conn = DBI->connect("dbi:PgPP:dbname=$dbname;host=localhost","$userName","$pwd") 
...
#  ---  get the Output Type for the run
   ...
   if ($output eq "VOC" || $output eq "")
   {
	...
	$qa = "";
   	...
   	if ($qa eq "") 
	{ $conn->disconnect(); print "ERROR: Run type is not set 'CRITERIA/INTEGRATE/NONINTEGRATE/HAPLIST'\n"; exit 10 }
    }
   if ($output eq "PM")
   {
	## check mechanism
