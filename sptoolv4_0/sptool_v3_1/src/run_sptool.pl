...
# -- Get the directory for the speciation tool source code
my $plDir = $ENV{"PERL_BIN"};
unless ($plDir) {
    die "\nError: Undefined PERL_BIN environment variable (PERL installation path) is required."
}
...
if ($use_env eq "t") {
    ...
    $impStr = "${plDir}/perl -w ${srcDir}/import_control.pl $dbName $runName";
...
# --- check that the user specified control file exists ---
    ...
    $impStr = "${plDir}/perl -w ${srcDir}/import_control.pl $dbName $runName $ctlFile";
    $chkerrcode = system($impStr);
...
if ($sth->rows > 0)  
{
    print("Warning found in PM processing - review log file for keyword WARNING \n");
...
print("Completed output preparations\n");
...
$impStr = "${plDir}/perl -w ${srcDir}/write_outputs.pl $dbName $runName";
system ($impStr);
